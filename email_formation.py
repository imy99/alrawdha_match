...
def intiation_email(to_email,name, user_id, pdf_file):
    ...
    subject = "🎉 Welcome to Al Rawdha! Your Matrimonial Profile is Ready"
...
def error_email(to_email, name, user_id, pdf_file):
    ...
    """
    yag = yagmail.SMTP(GMAIL_USER, GMAIL_APP_PASSWORD)
    subject = "⚠️ Al Rawdha Matrimonial Profile Error"
    body = f"""Assalamu Alaykum {name},
...
def ammendment_email(to_email, user_id, pdf_file):
    """Send email with ID and PDF attachment"""
    yag = yagmail.SMTP(GMAIL_USER, GMAIL_APP_PASSWORD)
    subject = "📝 Al Rawdha Profile Updated Successfully"
